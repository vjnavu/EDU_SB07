...
function dropCompare(dragObj, x, y) {
  ...
  for (var i = 0; i < dragDropManager.dropAreas.length; i++) {
    ...
    var dropValue = dropArea.getAttribute('answervalue');
    if (dragObjValue == dropValue) {
      var dropLeft = getRealOffsetLeft(dropArea) * zoomRate;
      var dropTop = getRealOffsetTop(dropArea) * zoomRate;
      var dropWidth = dropArea.clientWidth * zoomRate
      var dropHeight = dropArea.clientHeight * zoomRate;
      var dropIdx = dropArea.getAttribute('dropIdx');

      if (dragObjLeft > dropLeft + (dropWidth / 2)) {
        return false;
      } else {
        if (dragObjTop > dropTop + (dropHeight / 2) || dragObjTop < dropTop - (dropHeight / 2)) {
          return false;
        } else {
...
$(function () {
  $(".openpop").click(function () {
    ...
    if ($(this).hasClass("cha_btn") || $(this).hasClass("cha_btn1")) {
    } else {
      // ui 숨기는 코드
      if (!$("#" + target).hasClass('txtpopup')) {
        var offset = $("#" + target).offset(); // remote
        if (offset.left === 0 && offset.top === 0) { // remote
  ...
  $(".popup .popclose").click(function () {
    ...
    if (parent.document.querySelector('iframe')) {
      var parentDocument = parent.document.querySelector('iframe');
      parentDocument.parentNode.style.position = '';
      parentDocument.parentNode.style.zIndex = '';
    }
  });
});