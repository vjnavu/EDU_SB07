    ...
    start: function () {
      setTimeout(function(){
        if($ts.getEl('[data-open-gate]')) {
          $ts.loadScriptFile('../math/contents/js/cmn_openGate.js', function(){
            openGateInit();
          });
        }
        if($ts.getEl('[data-addLine-container]')) {
          $ts.loadScriptFile('../math/contents/js/quiz/quiz_addLine.js', function(){
            addLineInit();
          });
        }
        if($ts.getEl('[data-puzzle-container]')) {
          $ts.loadScriptFile('../math/contents/js/quiz/quiz_math_word.js', function(){
            puzzleQuizInit();
          });
        }
        if($ts.getEl('[data-calc-container]')) {
          $ts.loadScriptFile('../math/contents/js/quiz/quiz_math_calc.js', function(){
            mathCalculationInit();
          });
        }
        ...
        if($ts.getEl('.wordQuizContent').length) {
          $ts.loadScriptFile('../math/contents/js/quiz/quiz_life_wordQuiz.js', function(){
            wordQuizHint();
          });
        }
        if($ts.getEl('.fishing').length) {
          $ts.loadScriptFile('../math/contents/js/quiz/quiz_life_fishingGame.js', function(){
            fishingGame();
          });
        }
      }, 50);
