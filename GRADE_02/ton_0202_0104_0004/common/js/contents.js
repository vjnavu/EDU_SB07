  ...
  var contents = {
    start: function () {
      setTimeout(function () {
        ...
        //이미지 줌
        // if (QSAll('.zoomContainer .zoomImgContainer').length > 0) {
        //   // TODO: 리셋 버튼 추가
        //   // imgZoomInit();
...
function popupBoxEv() {
  ...
  //bgSound: proto_commond.js
  if (bgSound) {
    if (!this.classList.contains("noSound")) {
      efSound('media/click.mp3');
    }
  }
  ...
  if (this.classList.contains("popChest")) {
    if (document.querySelector("#effectSoundControl").classList.contains("active")) {
      efSound("./media/proto_ton_008/chest.wav");
    }
    const tagImg = document.createElement("img");
    tagImg.classList.add("chestImg");
    tagImg.setAttribute("src", `./common/images/proto_ton_008/chest.gif?random=${Math.random()}`);
    document.querySelector("#quiz").insertAdjacentElement("beforeend", tagImg);

    setTimeout(() => {
      tagImg.remove();
    }, 2800);
  }
  if (document.querySelector(".background-dark")) {
    const backgroundDark = document.querySelector(".background-dark");
    backgroundDark.classList.remove("hide");
  }
  ...
  if (
    ...
    if (!popbox.classList.contains('noReset')) {
      popupCloseBtn = createElement('div', popbox, 'popCloseBtn');
    }
    if (!popbox.classList.contains('game')) {
      alignPopup();
  ...
  popupCloseBtn.addEventListener('click', function () {
    ...
    if (!popbox.classList.contains('pageView')) {
      nonePage(popbox);
    }
  });
}
...
function nonePage(targetArea) {
  ...
  // 정답(추가) : 초기화
  if (targetArea.querySelectorAll('.answerView').length) {
    var answerViews = targetArea.querySelectorAll('.answerView');
    var ansViewLen = answerViews.length;
    var answerView = null;
    for (i = 0; i < ansViewLen; i++) {
      answerView = answerViews[i];
      answerView.classList.remove('ansView');
    }
  }
  ...
  if (curTab !== null) {
    ...
    if (isThere('.charSpriteAni')) {
      $prite.allresetSprite();
      $priteChange.resetSprite();
    }
...
function thinkOpen_Type() {
  ...
  if (thinkopen_Array.length > 1) {
    ...
    for (i = 0; i < thinkopen_Array.length; i++) {
      tabHTML += '<li class="tab" num="' + i + '">' + Number(i + 1) + '</li>';
    }
    tabHTML += '</ul>';
  ...
  thinkopen_btn.addEventListener(
    ...
    function () {
      if (thinkopen_popup_content.style.visibility == 'visible') {
        ...
        thinkopen_popup_tab.style.display = 'none';
        ...
        thinkopen_btn.classList.add('show');
        thinkopen_popup_tab.style.display = 'block';
...
function todaycontent_Type() {
  ...
  var typeText = createElement('em', pageParagraph);
  ...
  typeText.textContent = todayContents.typePage;
...
//정답 오픈 박스 추가 스크립트
$(function () {
  var $index = $(document).find(".answerView");

  $index.each(function () {
    if ($(this).data("spell")) {
      var $spellData = $(this).data("spell").split(",");
      $(this).addClass("spellSet");
      $(this).children(".answerText").before('<div class="spellBox">');
      for (var i = 0; i < $spellData.length; i++) {
        $(this).children(".spellBox").append('<span>' + $spellData[i] + '</span>');
      }
    } else if ($(this).hasClass("exm")) {
      $(this).children(".answerText").before('<div class="iconQuest">');
      $(this).children(".iconQuest").append('<span>예보기</span>');
    } else {
      $(this).children(".answerText").before('<div class="iconQuest">');
      $(this).children(".iconQuest").append('<span>?</span>');
    }

    $(this).on("click", function () {
      efSound('./media/click.mp3');

      var $popNum = $(this).find(".popBtn").attr("data-pop");
      var $popSet = $(document).find(".popbox[data-pop='" + $popNum + "']");
      var $popSetClose = $popSet.find(".popCloseBtn");

      if ($(this).hasClass("ansView")) {
        $(this).removeClass("ansView");
        if ($(this).find(".point").hasClass("view")) {
          $(this).find(".point").removeClass("view");
          $popSet.removeAttr("style").css("width", $popSet.data("width") + "px");
          $popSetClose.remove();
        }
      } else {
        $(this).addClass("ansView");
      }
    });

    $(this).find(".popBtn").on("click", function () {
      var $popNum = $(this).attr("data-pop");

      if ($(this).parent(".point").hasClass("view")) {
        $(this).parent(".point").removeClass("view");
        $(this).parents(".answerView").addClass("ansView");
      } else {
        $(this).parent(".point").addClass("view");
        $(this).parents(".answerView").removeClass("ansView");
      }
    });
  });

  $(document).on("click", ".popCloseBtn", function () {
    var $ansPoint = $(document).find(".ansView .point");

    if ($ansPoint.length > 0) {
      $ansPoint.removeClass("view");
    }
  });
});
...
//탭인탭 스크립트
$(function () {
  $(document).on("click", ".tabContainer", function () {
    if ($(this).siblings(".pageContainer").find('.subtab').length) {
      var subtabs = $(this).siblings(".pageContainer").find('.subtab');
      var subtabsConts = subtabs.siblings(".subtabcontents");

      subtabs.find(".subtablinks").removeClass("active");
      subtabs.find(".subtablinks:nth-child(1)").addClass("active");
      subtabsConts.find(".subtabcontent").removeClass("active");
      subtabsConts.find(".subtabcontent:nth-child(1)").addClass("active");
    }
  });

  $(".subtablinks").click(function () {
    efSound("./media/click.mp3");
    $('.answerView').removeClass('ansView');
    let subtabs = $(this).attr("data-sub-tabs");
    let subtab = $(this).attr("data-sub-tab");
    $(subtabs).find(".subtabcontent").removeClass("active");
    $(subtabs).find(subtab).addClass("active");
...
//버튼 클릭음 스크립트
$(function () {
  $(document).on("click", ".learningpaperBtn", function () {
    efSound('./media/click.mp3');
  });
  $(document).on("click", ".bluDictionary", function () {
    efSound('./media/click.mp3');
  });
